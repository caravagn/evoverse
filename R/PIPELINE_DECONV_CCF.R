                                                ...
                                                ...)
{
  ...
  CCF_karyotypes = intersect(karyotypes, names(CNAqc_input$n_karyotype))

  # Compute CCF
  CNAqc_input = CNAqc::compute_CCF(CNAqc_input, karyotypes = CCF_karyotypes, method =  ccf_method)

  print(CNAqc_input)
  cli::cli_process_done()

  CCF_estimates = CNAqc::CCF(CNAqc_input) %>% dplyr::filter(is.na(CCF))
  ...
  mobster_fits = list(`CCF` = CCF_fit)

  #
  # 3) BMix -- Binomial model -- with special adjustment for CCF
  #
  bmix_best_fits = deconvolution_nonread_counts_bmix_CCF(x = mobster_fits,
                                                         min_muts =  min_muts)
  ...
  results$log = paste0(
    "",
    Sys.time(),
    '. evoverse pipeline for subclonal deconvolution from CCF data. QC: ',
